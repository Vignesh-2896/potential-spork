...
function Homepage() {
  ...
  let updateRestaurantListData = (tempRestList) => {
    // For each restaurant to be displayed, the array is modified with an Item Box to hold its contents.
    let rList = [];
    tempRestList.forEach(function (item) {
      rList.push(
        <Link key={item.id} to={`restaurants/${item.id}`}>
          <RestaurantItem itemData={item} />
        </Link>
      );
    });
    if (tempRestList.length === 0 && rList.length === 0)
      rList.push(
        <h3 key="restaurant-not-found" className="restaurant-not-found">
          Oops ! No Restaurants found based on your filters. Clear all and try
          again !
        </h3>
      );
  ...
  let updateCategoryListData = () => {
    // Function to add available categories.
    let categoryList = [];
    let tempCategList = [];
  ...
  let applyFilters = () => {
    // Based on the available filters, restaurants will be filtered.

    CloseFilter();

    let resData = apiData.current;
    if (filterCuisine.length >= 1)
      resData = applySpecificFilter(
        resData,
        "restaurantCuisine",
        filterCuisine
    ...
    if (sortWithOpen)
      resData = applySpecificFilter(resData, "isOpen", sortWithOpen); // Filter Restaurants based on Open or Closed.
  ...
  let applySpecificFilter = (resData, key, requiredValues) => {
    ...
    resData.forEach(function (item) {
      let data = item[key];
      if (key === "restaurantCuisine" || key === "restaurantCategory")
        data = JSON.parse(data).flat();
      if (key === "isOpen") {
        if (data === true) tempRestList.push(item);
      } else {
        data.forEach(function (arrItem) {
          //Check if category/cuisine available in the restaurant is of the required value and if it has not been added before to our temporary list.
          if (
            requiredValues.indexOf(arrItem) !== -1 &&
            tempRestList.indexOf(item) === -1
          )
            tempRestList.push(item);
        });
      }
    });
    return tempRestList;
      ...
      {isLoading ? ( // If Fetch has not been complete, a Loading animation will be displayed.
        ...
        <div className="homepage">
          <div className="filterSection">
            ...
            <SideMenu />
            <span id="filterMenuBtn" className="filterMenuBtn">
              <img alt="FilterIcon" src={FilterIcon} onClick={ShowFilter} />
            </span>
          ...
          <div className="listSection">
            ...
            <div className="list-restaurants">
              ...
              {restaurantList.map(function (item, index) {
                // Iterating through each restaurant based on filters.
                return item;
              })}
            </div>
          </div>
