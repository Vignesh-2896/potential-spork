...
const Restaurant = () => {
    ...
    let updateRestaurantCategory = (CategoryList) => { // Function to add the available categories for the restaurant.
    ...
    let updateAllowedDishes = (DishesList) => {     // Function to add the allowed dishes in the restaurant.
    ...
    let filterWithCategory = (e) => {       // Filter the available dishes based on Food Category.
    ...
    return (  
        <div>
            {isError
            ? (<h3 class = "restaurant-error">No Restaurant Found ! Way back home is  <Link to ="/">Here !</Link> </h3>)
            : (
            <div>
                    ...
                    <div className = "restaurantPage">     
                        ...
                        <div className = "restaurantPage-about">
                            <div className = "restaurantPage-info">
                                <h3 className = "restaurantPage-info-name">{restaurantData.restaurantName}</h3>
                                <p className = "restaurantPage-info-summary">{restaurantData.restaurantDescription}</p>
                                <p className = "restaurantPage-info-opening"><img alt = "Time Icon" style = {{marginRight:"15px"}} src = {TimeIcon}/>{restaurantData.openingHours}</p>
                                <p className = "restaurantPage-info-phone"><img alt = "Call  Icon" style = {{marginRight:"15px"}} src = {CallIcon}/>{restaurantData.contactNumber}</p>
                                <p className = "restaurantPage-info-website"><img alt = "Website Icon" style = {{marginRight:"15px"}} src = {WebsiteIcon}/><a rel = "noreferrer" target = "_blank" href = {restaurantData.websiteUrl}>{restaurantData.websiteUrl}</a></p>
                            </div>
                        ...
                        <div className = "restaurantPage-menu">
                            <h3>Categories</h3>
                                {restaurantCategory.map(function(item,index){   // Iterating through each of the available categories.
                                    return <button key = {index} onClick = {filterWithCategory} className = "categoryItem">{item}</button>
                                })}
                            <h3>Menu</h3>
                            <div className = "restaurantPage-dishes">
                                {allowedDishes.map(function(item,index){        // Iterating through each of the dishes based on filters.
                                    return item;
                                })}
                            </div>
                        </div>
